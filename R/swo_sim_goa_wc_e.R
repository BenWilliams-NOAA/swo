...
swo_sim_goa_wc_e <- function(iters = 1, lfreq_data, specimen_data, cpue_data, strata_data, species, 
  ...
  lfreq_data %>% 
    ...
    tidytable::mutate.(region = ifelse(summary_area %in% c(919, 929, 939), "WC", 
  ...
  og_wc <- swo(lfreq_data = subset(.lfreq_data, .lfreq_data$region == "WC"), 
            specimen_data = subset(.specimen_data, .specimen_data$region == "WC"), 
            cpue_data = subset(.cpue_data, .cpue_data$region == "WC"),  
  ...
  # write og results 
  oga_wc %>% 
    tidytable::mutate.(region = "WC") -> .oga_wc
  oga_e %>% 
    tidytable::mutate.(region = "E") %>% 
    tidytable::bind_rows.(.oga_wc) -> oga
  
  ogl_wc %>% 
    tidytable::mutate.(region = "WC") -> .ogl_wc
  ogl_e %>% 
    tidytable::mutate.(region = "E") %>% 
    tidytable::bind_rows.(.ogl_wc) -> ogl
  
  if(isTRUE(save_orig)){
    vroom::vroom_write(oga, file = here::here("output", region, paste0(save, "_orig_age_wc_egoa.csv")), delim = ",")
    vroom::vroom_write(ogl, file = here::here("output", region, paste0(save, "_orig_length_wc_egoa.csv")), delim = ",")
  }
  
  # run iterations for western/central goa
  rr <- purrr::rerun(iters, swo(lfreq_data = subset(.lfreq_data, .lfreq_data$region == "WC"), 
                                specimen_data = subset(.specimen_data, .specimen_data$region == "WC"), 
                                cpue_data = subset(.cpue_data, .cpue_data$region == "WC"), 
  ...
  if(isTRUE(write_comp)) {
    r_age_wc %>%
      tidytable::map_df.(., ~as.data.frame(.x), .id = "sim") %>% 
      tidytable::mutate.(region = "WC") -> .r_age_wc
    r_age_e %>%
      tidytable::map_df.(., ~as.data.frame(.x), .id = "sim") %>% 
      tidytable::mutate.(region = "E") %>% 
      tidytable::bind_rows.(.r_age_wc) %>% 
      vroom::vroom_write(here::here("output", region, paste0(save, "_comp_age_wc_egoa.csv")), delim = ",")
    r_length_wc %>%
      tidytable::map_df.(., ~as.data.frame(.x), .id = "sim") %>% 
      tidytable::mutate.(region = "WC") -> .r_length_wc
    r_length_e %>%
      tidytable::map_df.(., ~as.data.frame(.x), .id = "sim") %>% 
      tidytable::mutate.(region = "E") %>% 
      tidytable::bind_rows.(.r_length_wc) %>% 
      vroom::vroom_write(here::here("output", region, paste0(save, "_comp_size_wc_egoa.csv")), delim = ",") 
  }
