...
#'
#' @examples
swo_sim_goa_cmplx <- function(iters = 1, lfreq_data, specimen_data, cpue_data, strata_data, 
  ...
  # subset data to goa rebs complex species
  goa_cmplx_species <- c(30050, 30051, 30052)
  
  lfreq_data %>% 
    tidytable::filter.(species_code %in% goa_cmplx_species) -> lfreq_data
    
  specimen_data %>% 
    tidytable::filter.(species_code %in% goa_cmplx_species) -> specimen_data
  
  cpue_data %>% 
    tidytable::filter.(species_code %in% goa_cmplx_species) -> cpue_data
  ...
  oga %>% 
    tidytable::summarize.(males = sum(males),
                       ...
                       unsexed = sum(unsexed),
                       .by = c(year, age)) %>% 
    tidytable::mutate.(species_code = "REBS") -> oga_cmplx
  ogl <- og$length
  ogl %>% 
    tidytable::summarize.(males = sum(males),
                          females = sum(females),
                          unsexed = sum(unsexed),
                          .by = c(year, length)) %>% 
    tidytable::mutate.(species_code = "REBS") -> ogl_cmplx
  
  if(isTRUE(save_orig)){
    vroom::vroom_write(oga, file = here::here("output", region, paste0(save, "_orig_age_goa_cmplx.csv")), delim = ",")
    vroom::vroom_write(ogl, file = here::here("output", region, paste0(save, "_orig_length_goa_cmplx.csv")), delim = ",")
  }
  ...
  if(isTRUE(write_comp)) {
    r_age %>%
      tidytable::map_df.(., ~as.data.frame(.x), .id = "sim") %>% 
      vroom::vroom_write(here::here("output", region, paste0(save, "_comp_age_goa_cmplx.csv")), delim = ",")
    r_length %>%
      tidytable::map_df.(., ~as.data.frame(.x), .id = "sim") %>% 
      vroom::vroom_write(here::here("output", region, paste0(save, "_comp_size_goa_cmplx.csv")), delim = ",")
  }
  ...
  r_age_cmplx %>%
    tidytable::map.(., ~ess_age(sim_data = .x, og_data = oga_cmplx)) %>% 
    tidytable::map_df.(., ~as.data.frame(.x), .id = "sim") -> ess_age

  r_length_cmplx %>%
    tidytable::map.(., ~ess_size(sim_data = .x, og_data = ogl_cmplx)) %>%
    tidytable::map_df.(., ~as.data.frame(.x), .id = "sim") -> ess_size
  
  if(!is.null(save)){
    vroom::vroom_write(ess_age, 
                       here::here("output", region, paste0(save, "_ess_ag_goa_cmplx.csv")), 
                       delim = ",")
    vroom::vroom_write(ess_size, 
                       here::here("output", region, paste0(save, "_ess_sz_goa_cmplx.csv")), 
                       delim = ",")
  }
