#' replicate swo function for spatially-explicit iss for western and central-eastern gulf subregions
#'
...
swo_sim_goa_w_ce <- function(iters = 1, lfreq_data, specimen_data, cpue_data, strata_data, species, 
  ...
  lfreq_data %>% 
    ...
    tidytable::mutate.(region = ifelse(summary_area %in% c(919), "W", 
                                        ifelse(summary_area %in% c(929, 939, 949, 959), "CE", NA))) %>% 
  ...
  og_ce <- swo(lfreq_data = subset(.lfreq_data, .lfreq_data$region == "CE"), 
               specimen_data = subset(.specimen_data, .specimen_data$region == "CE"), 
               cpue_data = subset(.cpue_data, .cpue_data$region == "CE"),  
               strata_data = strata_data, yrs = yrs, strata = FALSE,
               boot_hauls = FALSE, boot_lengths = FALSE, 
               boot_ages = FALSE, reduce_lengths = NULL, length_samples = NULL, 
               sex_samples = NULL)
  ...
  oga %>% 
    tidytable::summarize.(males = sum(males),
                          females = sum(females),
                          unsexed = sum(unsexed),
                          .by = c(year, age, species_code)) -> oga_ce
  ogl <- og_ce$length
  ogl %>% 
    tidytable::summarize.(males = sum(males),
                          females = sum(females),
                          unsexed = sum(unsexed),
                          .by = c(year, length, species_code)) -> ogl_ce
  
  # write og results 
  oga_w %>% 
    tidytable::mutate.(region = "W") -> .oga_w
  oga_ce %>% 
    tidytable::mutate.(region = "CE") %>% 
    tidytable::bind_rows.(.oga_w) -> oga
  
  ogl_w %>% 
    tidytable::mutate.(region = "W") -> .ogl_w
  ogl_ce %>% 
    tidytable::mutate.(region = "CE") %>% 
    tidytable::bind_rows.(.ogl_w) -> ogl
  
  if(isTRUE(save_orig)){
    vroom::vroom_write(oga, file = here::here("output", region, paste0(save, "_orig_age_w_cegoa.csv")), delim = ",")
    vroom::vroom_write(ogl, file = here::here("output", region, paste0(save, "_orig_length_w_cegoa.csv")), delim = ",")
  }
  ...
  # run iterations for central-eastern goa
  rr <- purrr::rerun(iters, swo(lfreq_data = subset(.lfreq_data, .lfreq_data$region == "CE"), 
                                specimen_data = subset(.specimen_data, .specimen_data$region == "CE"), 
                                cpue_data = subset(.cpue_data, .cpue_data$region == "CE"), 
                                strata_data = strata_data, 
                                yrs = yrs, strata = strata, 
  ...
  if(isTRUE(write_comp)) {
    ...
    r_age_ce %>%
      ...
      vroom::vroom_write(here::here("output", region, paste0(save, "_comp_age_w_cegoa.csv")), delim = ",")
    r_length_w %>%
      tidytable::map_df.(., ~as.data.frame(.x), .id = "sim") %>% 
    ...
    r_length_ce %>%
      ...
      vroom::vroom_write(here::here("output", region, paste0(save, "_comp_size_w_cegoa.csv")), delim = ",") 
  }
  ...
  r_age_w %>%
    tidytable::map.(., ~ess_age(sim_data = .x, og_data = oga_w, strata = strata)) %>% 
    tidytable::map_df.(., ~as.data.frame(.x), .id = "sim") -> ess_age_w
  
  r_length_w %>%
    tidytable::map.(., ~ess_size(sim_data = .x, og_data = ogl_w, strata = strata)) %>%
    tidytable::map_df.(., ~as.data.frame(.x), .id = "sim") -> ess_size_w
  
  r_age_ce %>%
    tidytable::map.(., ~ess_age(sim_data = .x, og_data = oga_ce, strata = strata)) %>% 
    tidytable::map_df.(., ~as.data.frame(.x), .id = "sim") -> ess_age_ce
  
  r_length_ce %>%
    tidytable::map.(., ~ess_size(sim_data = .x, og_data = ogl_ce, strata = strata)) %>%
    tidytable::map_df.(., ~as.data.frame(.x), .id = "sim") -> ess_size_ce
  
  ess_age_w %>% 
    tidytable::mutate.(region = "W") -> .ess_age_w
  ess_age_ce %>% 
    tidytable::mutate.(region = "CE") %>% 
    tidytable::bind_rows.(.ess_age_w) -> ess_age
    
  ess_size_w %>% 
    tidytable::mutate.(region = "W") -> .ess_size_w
  ess_size_ce %>% 
    tidytable::mutate.(region = "CE") %>% 
    tidytable::bind_rows.(.ess_size_w) -> ess_size  
  
  if(!is.null(save)){
    vroom::vroom_write(ess_age, 
                       here::here("output", region, paste0(save, "_ess_ag_w_cegoa.csv")), 
                       delim = ",")
    vroom::vroom_write(ess_size, 
                       here::here("output", region, paste0(save, "_ess_sz_w_cegoa.csv")), 
                       delim = ",")
  }
