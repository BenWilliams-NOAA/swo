...
swo_sim_goa_rebs <- function(iters = 1, lfreq_data, specimen_data, cpue_data, strata_data, 
  ...
  goa_rebs <- c(30050, 30051, 30052)
  
  lfreq_data %>% 
    tidytable::filter.(species_code %in% goa_rebs) -> lfreq_data
  
  specimen_data %>% 
    tidytable::filter.(species_code %in% goa_rebs) -> specimen_data
  
  cpue_data %>% 
    tidytable::filter.(species_code %in% goa_rebs) -> cpue_data
