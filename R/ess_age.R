  ...
  if (isTRUE(strata)) {
    ...
    sim_data %>% 
      ...
      tidytable::mutate.(ess_f = sum(og_f * (1 - og_f)) / sum((prop_f - og_f)^2),
                         ...
                         .by = c(year, species_code, stratum, type)) %>%
      tidytable::select.(year, species_code, type, stratum, age, ess_f, ess_m, ess_t) %>% 
      tidytable::pivot_longer.(cols = c(ess_f, ess_m, ess_t), names_to = "ess") %>%
      tidytable::distinct(year, species_code, type, stratum, ess, value) 
    ...
    sim_data %>% 
      ...
      tidytable::pivot_longer.(cols = c(ess_f, ess_m, ess_t), names_to = "ess") %>%
      tidytable::distinct(year, species_code, type, ess, value) -> t 
  }
